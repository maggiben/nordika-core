...
Creating or updating a contact with `projectId` or `projectIds` MUST merge into
the existing membership set without dropping unrelated project ids unless the
caller explicitly replaces `projectIds`.
