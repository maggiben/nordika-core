...
| `OPENAI_API_KEY` | No | When set, inbound catalog/task replies are parsed into structured progress via OpenAI (default provider). Account settings keys override this when present. |
| `OPENAI_PROGRESS_MODEL` | No | Chat model for OpenAI progress parsing (default `gpt-4o-mini`) |
| `ANTHROPIC_API_KEY` | No | When set and account `progressAi.provider` is `anthropic`, inbound progress uses Anthropic instead of OpenAI. Account settings keys override this when present. |
